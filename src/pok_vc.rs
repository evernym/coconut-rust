    ...
    ( $ProverCommitting:ident, $ProverCommitted:ident, $Proof:ident, $group_element:ident, $group_element_vec:ident ) => {
        ...
        impl $ProverCommitting {
            ...
            pub fn get_index(
                &self,
                idx: usize,
            ) -> Result<(&$group_element, &FieldElement), CoconutError> {
                if idx >= self.gens.len() {
                    return Err(CoconutError::GeneralError {
        ...
        impl $ProverCommitted {
            ...
            ) -> Result<$Proof, CoconutError> {
                if secrets.len() != self.gens.len() {
                    return Err(CoconutError::UnequalNoOfBasesExponents {
                        bases: self.gens.len(),
...
pub(crate) mod tests {
    use super::*;
    // XXX: Error for VC should be independent of Coconut
